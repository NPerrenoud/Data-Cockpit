...
public class CustomDeviationRenderer extends DeviationRenderer {
	...
	/**
	 * Returns the range of values the renderer requires to gui all the
	...
	protected void drawPrimaryLineAsPath(XYItemRendererState state,
		...
		for (int i = 0 ; i < 3 ; i++) {
			...
			if (item == dataset.getItemCount(series) - 1) {
				// draw path
				drawFirstPassShape(g2, series, item, p, i);
	...
	protected void drawFirstPassShape(Graphics2D g2, int series,
